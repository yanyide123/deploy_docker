...
class ShiYouDecordInit:
    ...
    def __call__(self, results):
        ...
        # print("loading_filename {}".format(results["filename"]))
        ...
        # while rval:
        #     rval, frame = vc.read()
        #     picture_list.append(np.array(frame))
        
        # io.BytesIO是经过UTF-8编码的bytes。
        # print("results['filename'] {}".format(results))
        # print("results['imgs'] {}".format(results['imgs']))
        # file_obj = io.BytesIO(self.file_client.get(results['filename']))
        # print("results['filename'] {}".format(results))
        # print("file_obj", file_obj)
        # print("results['filename']", results['filename'])
        # container = decord.VideoReader(file_obj, num_threads=self.num_threads)
        # print("container {}".format(type(container[0])))
        ...
        #         imgs.append(out_img)
        # print(out_img.shape)
        # mmcv.imwrite(out_img, os.path.join(jpg_path, "img_%05d.jpg" % i))
        
        # imgs = []
        # for img_namae in sorted(os.listdir(jpg_path)):
        #     img_path = os.path.join(jpg_path, img_namae)
        #     img = mmcv.imread(img_path)
        #     imgs.append(img)
        #
        
        # print(type(container))
        # container = np.array(picture_list)
        # print("loding_container {}".format(container))
        # for i in container:
        #     print(type(i))
        # print("type", type(container))
        # print("imgs {}".format(type(imgs[0])))
...
class ShiYouDecordDecode:
    ...
    def __call__(self, results):
        ...
        frame_dict = {
            # idx: container[idx].asnumpy()
            idx: container[idx] for idx in np.unique(frame_inds)
        }
