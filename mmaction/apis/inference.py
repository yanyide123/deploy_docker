...
def inference_recognizer_copy(model,
                              video_path,
                              label_path,
                              use_frames=False,
                              outputs=None,
    ...
    # with open(label_path, 'r') as f:
    #     label = [line.strip() for line in f]
    # label = [line for line in label_path]
    # label = label_path
    ...
    if use_frames:
        ...
        #     r"E:\kuangwuchenfen\daimajiaojie\data\output\ABC\videos\ABC_202111081751_004_10X_0.avi",
        #     0)
        # print(cap)
...
def init_recognizer_pth(model, checkpoint=None, device='cuda:0', use_frames=False):
    load_checkpoint(model, checkpoint, map_location=device)
    model.to(device)
    model.eval()
    return model
...
def inference_recognizer_pth(cfg, video_path, tensor, model, device="cuda:0"):
    test_pipeline = cfg.data.test.pipeline
    test_pipeline = Compose(test_pipeline)
    data = dict(filename=video_path, label=-1, start_index=0, modality='RGB')
    data["imgs_mapping"] = tensor
    data = test_pipeline(data)
    data = collate([data], samples_per_gpu=1)
    if device:
        # 使用gpu
    ...
    # print("images {}".format(type(imgs)))
    # imgs = imgs.astype(np.float32)
        # forward the model
    with torch.no_grad():
        result = model(return_loss=False, **data)[0]
    # result = model(None, {str(0): imgs})[0]
    # print("onnx_result {}".format(onnx_result))
    outputs = np.array(result)
    outputs_class = np.argmax(outputs, axis=-1)
    torch.set_printoptions(precision=8)
    outputs_score = result[outputs_class]
    return outputs_class, outputs_score


def inference_recognizer_onnx(cfg, video_path, tensor, ort_sess, device="cuda:0"):      # device="cuda:0"
    test_pipeline = cfg.data.test.pipeline
    test_pipeline = Compose(test_pipeline)
    data = dict(filename=video_path, label=-1, start_index=0, modality='RGB')
    data["imgs_mapping"] = tensor
    ...
    # print("images {}".format(type(imgs)))
    # imgs = imgs.astype(np.float32)
    onnx_result = ort_sess.run(None, {str(0): imgs})[0]
    # print("onnx_result {}".format(onnx_result))
    outputs = np.array(onnx_result)
    outputs_class = np.argmax(outputs, axis=-1)
    torch.set_printoptions(precision=8)
    outputs_score = F.softmax(torch.from_numpy(onnx_result), dim=1)[0, outputs_class[0]].item()
    return outputs_class, outputs_score


def inference_recognizer_video(cfg, video_path, ort_sess, device="cuda:0"):
    test_pipeline = cfg.data.test.pipeline
    test_pipeline = Compose(test_pipeline)
    data = dict(filename=video_path, label=-1, start_index=0, modality='RGB')
    data = test_pipeline(data)
